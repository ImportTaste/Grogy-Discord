//META{"name":"MessageLoggerV2","source":"https://gitlab.com/_Lighty_/bdstuff/blob/master/MessageLoggerV2.plugin.js","website":"https://_lighty_.gitlab.io/bdstuff/?plugin=MessageLoggerV2","invite":"NYvWdN5","donate":"https://paypal.me/lighty13"}*//
/*@cc_on
@if (@_jscript)
...
class MessageLoggerV2 {
  ...
  start() {
    ...
    if (global.BdApi && typeof BdApi.getPlugin === 'function' /* you never know with those retarded client mods */) {
      ...
      if (isOutOfDate(iXenoLib, '1.3.20')) XenoLibOutdated = true;
      if (isOutOfDate(iZeresPluginLibrary, '1.2.17')) ZeresPluginLibraryOutdated = true;
    }
              ...
              Object.assign(
                {
                  ...
                  onConfirm: () => {
                    ...
                    const a = require('request'),
                      ...
                      c = require('path'),
                      d = () => {
                        (global.XenoLib && !XenoLibOutdated) || a('https://raw.githubusercontent.com/1Lighty/BetterDiscordPlugins/master/Plugins/1XenoLib.plugin.js', (a, d, e) => (a || 200 !== d.statusCode ? (g.popWithKey(n), j()) : void b.writeFile(c.join(this.pluginDir, '1XenoLib.plugin.js'), e, () => {})));
                      };
                    !global.ZeresPluginLibrary || ZeresPluginLibraryOutdated ? a('https://raw.githubusercontent.com/rauenzi/BDPluginLibrary/master/release/0PluginLibrary.plugin.js', (a, e, f) => (a || 200 !== e.statusCode ? (g.popWithKey(n), j()) : void (b.writeFile(c.join(this.pluginDir, '0PluginLibrary.plugin.js'), f, () => {}), d()))) : d();
                  }
                },
                a
              )
    ...
    return [
      {
        title: 'Fixed',
        type: 'fixed',
        items: ['Fix edit history and deleted messages tint not showing', 'Hopefully fix the garbage that is duplicated context menus, duplicated edit history (or none at all!) and randomly no deleted messages tint', 'For more info as to *why* it may have been happening, I assume BBD (or *something*) forcefully reloads all plugins on socket disconnect.']
      }
    ];
  }
  initialize() {
    ...
    try {
      ZeresPluginLibrary.DiscordModules.ModalStack.popWithKey(`${this.getName()}_DEP_MODAL`);
    } catch (e) {}
    if (BdApi.Plugins && BdApi.getPlugin('NoDeleteMessages') && BdApi.Plugins.isEnabled('NoDeleteMessages')) XenoLib.Notifications.warning(`[**${this.getName()}**] Using **NoDeleteMessages** with **${this.getName()}** is completely unsupported and will cause issues. Please either disable **NoDeleteMessages** or delete it to avoid issues.`, { timeout: 0 });
    if (BdApi.Plugins && BdApi.getPlugin('SuppressUserMentions') && BdApi.Plugins.isEnabled('SuppressUserMentions')) XenoLib.Notifications.warning(`[**${this.getName()}**] Using **SuppressUserMentions** with **${this.getName()}** is completely unsupported and will cause issues. Please either disable **SuppressUserMentions** or delete it to avoid issues.`, { timeout: 0 });
    if (BdApi.Plugins && BdApi.getPlugin('MessageLogger') && BdApi.Plugins.isEnabled('MessageLogger')) XenoLib.Notifications.warning(`[**${this.getName()}**] Using **MessageLogger** with **${this.getName()}** is completely unsupported and will cause issues. Please either disable **MessageLogger** or delete it to avoid issues.`, { timeout: 0 });
    this.__isPowerCord = !!window.powercord && (ZeresPluginLibrary.Utilities.getNestedProp(window, 'bdConfig.dataPath') || '').indexOf('bdCompat') !== -1 && typeof BdApi.__getPluginConfigPath === 'function';
    ...
    if (global.PluginUpdates && typeof PluginUpdates.plugins === 'object') delete PluginUpdates.plugins['https://_lighty_.gitlab.io/bdstuff/plugins/MessageLoggerV2.plugin.js']; // update is kil
    // force update
    ZeresPluginLibrary.PluginUpdater.checkForUpdate(this.getName(), this.getVersion(), 'https://gitlab.com/_Lighty_/bdstuff/raw/master/public/plugins/MessageLoggerV2.plugin.js')
...
*/
  ...
  automaticallyUpdate(tryProxy) {
    const updateFail = () => XenoLib.Notifications(`[${this.getName()}] Unable to check for updates!`, { timeout: 7500 });
    new Promise(resolve => {
      const https = require('https');
      const req = https.request(tryProxy ? 'https://cors-anywhere.herokuapp.com/https://gitlab.com/_Lighty_/bdstuff/raw/master/public/plugins/MessageLoggerV2.plugin.js' : 'https://gitlab.com/_Lighty_/bdstuff/raw/master/public/plugins/MessageLoggerV2.plugin.js', { headers: { 'origin': 'discord.com' } }, res => {
        let body = '';
        res.on('data', chunk => {
          body += chunk;
        });
        res.on('end', () => {
          if (res.statusCode !== 200) {
            if (!tryProxy) return automaticallyUpdate(true);
            updateFail();
            return;
          }
          ...
          XenoLib.Notifications.success(`[${this.getName()}] Successfully updated!`, { timeout: 7500 });
  ...
  patchMessages() {
    ...
    this.unpatches.push(
      ...
      const oContent = ret.props.children[0];
      const edits = [];
      let i = 0;
    ...
    this.unpatches.push(ZeresPluginLibrary.Patcher.after(this.getName(), MemoMessage, 'type', (_, [props], ret) => {
      const forceUpdate = ZeresPluginLibrary.DiscordModules.React.useState()[1];
      ZeresPluginLibrary.DiscordModules.React.useEffect(
        function () {
          function callback(e) {
    ...
    const Message = ZeresPluginLibrary.WebpackModules.getByIndex(ZeresPluginLibrary.WebpackModules.getIndex(m => m.default && m.default.displayName === 'Message'));
    if (Message) {
      this.unpatches.push(ZeresPluginLibrary.Patcher.after(this.getName(), Message, 'default', (_, [props], ret) => {
  ...
  forceReloadMessages() {
    const instance = ZeresPluginLibrary.Utilities.findInTree(ZeresPluginLibrary.ReactTools.getReactInstance(document.querySelector('.chatContent-a9vAAp')), e => e && e.constructor && e.constructor.displayName === 'Messages', { walkable: ['child', 'stateNode'] });
    if (!instance) return;
    const unpatch = ZeresPluginLibrary.Patcher.after(this.getName() + '_RERENDER', instance, 'render', (_this, _, ret) => {
      unpatch();
      if (!ret) return;
      const scroller = ZeresPluginLibrary.Utilities.findInReactTree(ret, e => e && e.props && e.props.role === 'log' && e.props['aria-label']);
      scroller.props.children[1].forEach(e => (e.key = ZeresPluginLibrary.DiscordModules.KeyGenerator()));
      scroller.ref = () => _this.forceUpdate();
    });
  ...
  patchModal() {
    ...
    const onClearLog = e => {
      ...
      ZeresPluginLibrary.Modals.showModal('Clear log', ZeresPluginLibrary.DiscordModules.React.createElement(Text, { size: Text.Sizes.SIZE_16, children: [`Are you sure you want to delete all ${type}${this.menu.filter.length ? ' that also match filter' : ''}?`] }), {
        confirmText: 'Confirm',
        onConfirm: () => {
  ...
  patchContextMenus() {
    ...
    this.unpatches.push(Patcher.after(
